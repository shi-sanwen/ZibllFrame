 ...
 * @LastEditTime: 2023-11-12 19:36:16
 * @Project       : Zibll子比主题
...
function zib_get_avatar($avatar, $id_or_email, $args)
{
    ...
    $style =  '';
 if (function_exists('get_avatarbox_user_toshi')) {
...
$style =  '';
  ...
  return '<img alt="' . esc_attr($alt) . '" src="' . esc_url($avatar) . '"  '.$style.' class="avatar  ' . ($args['size'] ? ' avatar-' . $args['size'] : '') . ' avatar-id-' . $user_id . '"' . ($args['size'] ? ' height="' . $args['size'] . '" width="' . $args['size'] . '"' : '') . '>';
}
...
function _load_scripts()
{
    if (!is_admin()) {
        ...
        wp_enqueue_script('jquery', isset($jss[$js_outlink]['jquery']) ? $jss[$js_outlink]['jquery'] : $purl . '/js/libs/jquery.min.js', false, THEME_VERSION, false);
...
function is_disable_email($email)
{
    ...
    foreach ($disable_reg_keywords as $keyword) {
        if ($keyword && (stristr($email, $keyword) || $keyword == $email)) {
            return true;
        }
    }
    return false;
...
function add_shortcode_hidecontent($atts, $content = null)
{
    ...
    switch ($type) {
        case 'reply':
            $a     = '#commentform';
            $_hide = '<div class="hidden-box" reply-show="true" reload-hash="#hidden-box-comment"><a class="hidden-text" href="javascript:(scrollTo(\'' . $a . '\',-120));"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;此处内容已隐藏，请评论后刷新页面查看.</a></div>';
