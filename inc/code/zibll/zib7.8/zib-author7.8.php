 ...
 * @LastEditTime: 2024-04-09 19:57:27
...
function zib_author_header()
{
    ...
    if (function_exists('get_avatarbox_user_toshi')) {
        $avatar_img =   get_avatarbox_user_toshi($author_id);
       if ($avatar_img == true) {
              $cache = wp_cache_get($author_id, 'user_avatar', true);
        $avatar_img = get_avatarbox_user_toshi($author_id,$cache);
       }else{
           $avatar_img =   '';
       }
         }
    $info_html = '<div class="flex header-info relative hh">';
    $info_html .= '<div class="flex0 header-avatar"><span class="avatar-img">' .$avatar_img. $avatar . '</span></div>';
...
function zib_get_user_follow_count($user_id, $type = 'followed')
{
    ...
    $count = 0;
    $data  = $type === 'follow' ? maybe_unserialize(get_user_meta($user_id, 'follow-user', true)) : maybe_unserialize(get_user_meta($user_id, 'followed-user', true));
    ...
    //添加缓存，30天有效
    wp_cache_set($user_id, $count, 'user_' . $type . '_count', 2592000);
