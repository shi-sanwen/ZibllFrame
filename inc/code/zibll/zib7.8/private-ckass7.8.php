 ...
 * @LastEditTime: 2022-11-27 20:09:22
...
class Zib_Private
{
        ...
        foreach ($user_db as $user) {
            ...
            if ($user['send_user'] == $user_id) {
                ...
                $ajax_query_arg['receive_user'] = $user['receive_user'];
                            $avatar_img =  '';
          $avatar_id =   $user['receive_user'];
            if (function_exists('get_avatarbox_user_toshi')) {
          $avatar_img =   get_avatarbox_user_toshi($avatar_id,true);
                   }
            } else {
                //我是接收人
          ...
          $avatar_img =   get_avatarbox_user_toshi($avatar_id,true);
            ...
            $list .= '<div class="avatar-img shrink0 mr10">'. $avatar_img . $avatar . '</div>';
            ...
            $lists .= '<div class="padding-h10 border-bottom ' . $class . '"><a href="javascript:;"  data-toggle-class="toggle" data-target=".msg-private" ajax-target="#user_private_window" data-ajax="' . $ajax_url . '" ajax-replace="true">' . $list . '</a></div>';
        }
        //翻页按钮
    ...
    public static function get_msg_box($msg = array(), $user_id = '', $class = '')
    {
        ...
        $avatar = '<div class="avatar-img">' .$avatar_img. $avatar . '</div>';
